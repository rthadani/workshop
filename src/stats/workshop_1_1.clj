...
;## To run the notebook
;- Start the repl  
;- Evaluate the namespace clerk
;- 

^{:nextjournal.clerk/visibility #{:hide-ns}}
(ns stats.workshop-1-1
...
^:nextjournal.clerk/no-cache
(comment (->> (repeatedly 10000 #(apply-prob 0.5 17))
     (map (fn [s] {:x s :y 1}))
...
^:nextjournal.clerk/no-cache
(comment (->> (kstatsd/sample 10000 (kstatsd/binomial {:n 17 :p 0.5}))
     (map (fn [s] {:x s :y 1}))
...
; the mean where the population is n and the probability of success is p
...
(let [mu (* n p)
  ...
  (-> (ktest/test-result (/ (- o mu) sd) (kstatsd/normal {:location 0.0 :scale 1.0}))
     (ktest/p-value)))
...
;(experiment 56 450 (/ 48 550) 10000)
...
(comment (->> (repeatedly 10000 #(shuffled-avg-diff drug placebo))
              (map (fn [s] {:x s :y 1}))
...
(def placebo-measure-2 [56 348 162 420 440 250 389 476 288 456])
(def drug-measure-2 (mapv #(+ % (rand-int 50)) placebo-measure-2))

(avg-diff drug-measure-2 placebo-measure-2)
